  ...
  if (Serial.available() > 0) 
  {
    ...
    if (input.startsWith("firmware_Drone_")) 
    {
      ...
      if (underscoreCount == 6) 
      {
        ...
        for (int i = 0; i < 4; i++) 
        {
          if (motorValues[i] < 127)
          {
            int newMotorValue = map(motorValues[i], 0, 126, 255, 50);
            analogWrite(motorPins[i], newMotorValue);
            //Serial.print(newMotorValue);
            //Serial.println();
          }

          if (motorValues[i] > 127)
          {
            int newMotorValue = map(motorValues[i], 128, 255, 50, 255);
